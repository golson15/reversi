...
socket.on('join_room_response', function(payload) {
  ...
  /* If we are being notified that we joined the room, then ignore it; should be _*/
...
/*May include payload in function */
...
socket.on('game_update', function(payload) {
  ...
  /*Check for good board game_update */
  if (payload.result == 'fail'){
...
for(row =0; row <8; row++) {
  ...
  if(payload.game.whose_turn === my_color){
    if(payload.game.legal_moves[row][column] === my_color_move.substr(0,1)){
